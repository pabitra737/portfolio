...
export default function FloatingLines({
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      animationSpeed: { value: 1 }, // Default, updated via effect
    ...
    const handlePointerMove = event => {
      ...
      // Parallax target update (read latest parallaxStrength from ref or props if possible, but for now use uniform default)
      // Note: we'll use the values from uniforms which are updated by the other effect
      const pStrength = uniforms.parallaxStrength.value;
      const centerX = rect.width / 2;
      const centerY = rect.height / 2;
      const offsetX = (x - centerX) / rect.width;
      const offsetY = -(y - centerY) / rect.height;
      targetParallaxRef.current.set(offsetX * pStrength, offsetY * pStrength);
    };
    ...
    const renderLoop = () => {
      ...
      // Smooth mouse
      currentMouseRef.current.lerp(targetMouseRef.current, 0.05); // Fixed damping for smoothness
...
FloatingLines.propTypes = {
  linesGradient: PropTypes.arrayOf(PropTypes.string),
  enabledWaves: PropTypes.arrayOf(PropTypes.string),
  lineCount: PropTypes.oneOfType([PropTypes.number, PropTypes.arrayOf(PropTypes.number)]),
  lineDistance: PropTypes.oneOfType([PropTypes.number, PropTypes.arrayOf(PropTypes.number)]),
  topWavePosition: PropTypes.shape({ x: PropTypes.number, y: PropTypes.number, rotate: PropTypes.number }),
  middleWavePosition: PropTypes.shape({ x: PropTypes.number, y: PropTypes.number, rotate: PropTypes.number }),
  bottomWavePosition: PropTypes.shape({ x: PropTypes.number, y: PropTypes.number, rotate: PropTypes.number }),
  animationSpeed: PropTypes.number,
  interactive: PropTypes.bool,
  bendRadius: PropTypes.number,
  bendStrength: PropTypes.number,
